...
function parseCertificate(cert: string): ArrayBuffer {
  ...
  // A simplified ASN.1 parser to find the SubjectPublicKeyInfo
  let offset = 0;
  ...
  expectTag(0x30); // SEQUENCE (tbsCertificate)
  ...
  while (offset < tbsEnd) {
      ...
      if (tag === 0x30 && der[offset+2] === 0x0D && der[offset+3] === 0x06) {
          // Found the start of SubjectPublicKeyInfo based on OID for RSA encryption
          break;
      }
  ...
  // Go back to the start of the SubjectPublicKeyInfo sequence tag
  while(der[offset] !== 0x30) {
      offset--;
  }

  // We need the full SPKI structure including the tag and length
