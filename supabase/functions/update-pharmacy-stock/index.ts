...
async function updateStockInSheet(accessToken: string, stockUpdates: StockUpdate[]) {
  ...
  const stockIndex = headers.findIndex((h: string) => h.toLowerCase().trim() === 'stock');
  ...
  for (const update of stockUpdates) {
    ...
    const rowIndex = values.findIndex((row: string[], index: number) => 
      index > 0 && row[nameIndex]?.toLowerCase().trim() === update.name.toLowerCase().trim()
    );
...
const handler = async (req: Request) => {
  ...
  try {
    ...
    const stockUpdates: StockUpdate[] = items.map((item: any) => ({
      name: item.name,
