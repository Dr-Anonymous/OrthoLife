...
serve(async (req) => {
  ...
  try {
    ...
    const copyResponse = await fetch(`https://www.googleapis.com/drive/v3/files/${templateId}/copy`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${accessToken}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ name: data.name, parents })
    });
    ...
    if (data.dob) {
      ...
      age = (today.getFullYear() - dob.getFullYear() - (today < new Date(today.getFullYear(), dob.getMonth(), dob.getDate()) ? 1 : 0)).toString();
    }
    ...
    if (newFolderId && !folderId && templateId === "1Wm5gXKW1AwVcdQVmlekOSHN60u32QNIoqGpP_NyDlw4") {
