...
const PostComposer = () => {
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      let result;
      // ALWAYS call the publishing function if there's media, regardless of platforms, 
      // because the phone bridge needs the public URLs generated by the Edge function.
      if (apiPlatforms.length > 0 || (isPhoneBridgeSelected && mediaFiles.length > 0)) {
        result = await socialService.publishAll({
          content,
          platforms: apiPlatforms.length > 0 ? apiPlatforms : ['phone_bridge_only'],
