...
export const Prescription: React.FC<PrescriptionProps> = React.forwardRef<HTMLDivElement, PrescriptionProps>(({ patient, consultation, consultationDate, age, language, logoUrl }, ref) => {
  ...
  return (
    <div ref={ref} className="font-sans text-sm bg-background text-foreground" style={{ fontFamily: 'var(--font-sans)' }} data-testid="prescription">
      <table className="w-full prescription-table">
        {/* Header */}
        <thead>
          <tr>
            <td>
              <header
                className="px-8 flex justify-between items-center pb-4 border-b-2 border-primary-light rounded-t-lg"
        ...
        <tfoot>
          <tr>
            <td>
              <footer
                className="mt-8 py-3 px-8 border-t-2 border-primary-light rounded-b-lg flex justify-between items-center prescription-footer"
                style={{ backgroundImage: backgroundPattern }}
              >
